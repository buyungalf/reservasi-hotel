...
if(isset($_POST['signup'])){
    ...
    if(isset($_POST['check-email'])){
        ...
        if(mysqli_num_rows($run_sql) > 0){
            $code = rand(999999, 111111);
            $insert_code = "UPDATE tb_user SET kode = $code WHERE email = '$email'";
            $run_query =  mysqli_query($koneksi, $insert_code);
            if($run_query){
                $subject = "Password Reset Code";
                $message = "Your password reset code is $code";
                $sender = "From: buyung.fajrin@gmail.com";
                if(mail($email, $subject, $message, $sender)){
    ...
    if(isset($_POST['check-reset-otp'])){
        $_SESSION['info'] = "";
        $otp_code = mysqli_real_escape_string($koneksi, $_POST['otp']);
        $check_code = "SELECT * FROM tb_user WHERE kode = $otp_code";
        $code_res = mysqli_query($koneksi, $check_code);
        if(mysqli_num_rows($code_res) > 0){
            $fetch_data = mysqli_fetch_assoc($code_res);
            $email = $fetch_data['email'];
    ...
    if(isset($_POST['change-password'])){
        ...
        if($password !== $cpassword){
            ...
            $update_pass = "UPDATE tb_user SET kode = $code, password = '$encpass' WHERE email = '$email'";
