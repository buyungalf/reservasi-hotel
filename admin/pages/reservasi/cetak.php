<?php
    ...
    $query = mysqli_query($koneksi, "SELECT * FROM tb_tamu where id_tamu = '$id_tamu'");
    while($d=mysqli_fetch_array($query)){
        ...
        $no_id = $d['no_id'];}
    $query = mysqli_query($koneksi, "SELECT * FROM tb_tamu where no_trans = '$no_trans_'");
    while($d=mysqli_fetch_array($query)){
        $id_tamu = $d['id_tamu'];
        $id_kamar = $d['id_kamar'];
        $diskon = $d['diskon'];
        $i_checkin = date("j/n/Y | H:i",$d['checkin']);
        $i_checkout = date("j/n/Y | H:i");
    ...
    while($e=mysqli_fetch_array($query1)){
        ...
        //kondisi
        $lama_sewa = floor((mktime(date("H"),date("i"),date("s"),date("m"),date("d"),date("Y")) - mktime($get_jam_in,$get_menit_in,$get_detik_in,$get_bulan_in,$get_tanggal_in,$get_tahun_in))/86400);
        if ($lama_sewa <=0){
          $harga_kamar = $harga_k;
        }else{
          if ($get_jam_skr >= $get_13 | $get_jam_skr <= $get_15)
          {
            $harga_kamar = ($harga_k + ((25/100) * $harga_k)) * $lama_sewa;
          }
          else if ($get_jam_skr > $get_15 | $get_jam_skr <= $get_17)
          {
            $harga_kamar = ($harga_k + ((50/100) * $harga_k)) * $lama_sewa;
          }
    ...
    ?>
<?php
    $kueri1 = mysqli_query($koneksi, "SELECT SUM(biaya) FROM tb_tamu_order where id_tamu = $id_tamu");
              ...
              <div class="card-body p-0">
                ...
                <table border="1" cellpadding="10" cellspacing="0">
                    ...
                    <tr>
                      ...
                      <td>Rp.<?= number_format($harga_k,0,',','.'); ?></td>
                ...
                <table border="1" cellpadding="10" cellspacing="0">
                    ...
                    <?php										
					          $no=0;
					          $tot_biaya = 0;
					          $kueri = mysqli_query($koneksi,"select b.tgl_order, c.nm_order, c.harga, b.banyak, b.biaya from tb_tamu_order b, tb_order c where b.id_order = c.id and b.id_tamu = $id_tamu order by b.tgl_order desc");
